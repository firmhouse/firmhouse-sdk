...
export async function initializeCart() {
  const subscriptionToken = cookies().get(SUBSCRIPTION_TOKEN_COOKIE)?.value ?? undefined
...
export async function addToCart(data: FormData) {
  ...
  await firmhouseClient.subscriptions.addToCart({ productId, quantity }, await getSubscriptionToken())
  revalidatePath('/')
}

export async function removeFromCart(data: FormData) {
  const id = data.get('orderedProductId') as string
  await firmhouseClient.subscriptions.removeFromCart(id, await getSubscriptionToken())
  revalidatePath('/')
}
...
export async function updateQuantity(data: FormData) {
  ...
  await firmhouseClient.subscriptions.updateOrderedProductQuantity(id, quantity, await getSubscriptionToken())
  revalidatePath('/')
}