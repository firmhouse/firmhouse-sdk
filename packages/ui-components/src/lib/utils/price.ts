...
export function getOrderedProductInfo(orderedProduct: OrderedProductType, currency = 'EUR', locale?: string | null) {
    ...
    const shippingText = intervalUnitOfMeasure === 'only_once' ? '' : getShippingInterval(orderedProduct);
    const totalPrice = orderedProduct.totalAmountIncludingTaxCents ?? 0;
    const formattedPrice = `${formatCentsWithCurrency(totalPrice, currency, locale)}${intervalText}`;
    const billingAndShipping = isPlanBased ? shippingText : `${shippingText ? `Bills & `: ''}${shippingText}`;
...
function getShippingInterval(orderedProduct: OrderedProductType) {
    ...
    const {intervalUnitOfMeasure, interval} = orderedProduct.intervalUnitOfMeasure === 'default' ? orderedProduct.product : orderedProduct;
    if (intervalUnitOfMeasure === 'only_once' || typeof intervalUnitOfMeasure !== 'string' || typeof interval !== 'number') {
        return 'Ships once';
    }
