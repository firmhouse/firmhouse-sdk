...
describe('lib/resources/products/index.ts', () => {
    ...
    describe('fetchAll', () => {
        it('should call the correct query', async () => {
            const graphQLClient = new GraphQLClient('test', 'http://test.com')
            graphQLClient.request = jest.fn().mockResolvedValue({ products: { nodes: [] } })
            const testResource = new ProductsResource(graphQLClient)
        ...
        it('should parse parameters correctly', async () => {
            ...
            graphQLClient.request = jest.fn().mockResolvedValue({ products: { nodes: [] } })
            const testResource = new ProductsResource(graphQLClient)
            const params = { after: 'test', before: 'test', first: 10, last: 10, id: "test", shopifyVariantId: "test", sku: "test", updatedSince: "2020-01-01" }
            await testResource.fetchAll(params)
            expect(graphQLClient.request).toHaveBeenCalledWith(AllProductsDocument, params)
        })

        it('should filter the null values', async () => {
            const graphQLClient = new GraphQLClient('test', 'http://test.com')
            graphQLClient.request = jest.fn().mockResolvedValue({ products: { nodes: [{id: 'test'}, null] } })
            const testResource = new ProductsResource(graphQLClient)
    ...
    describe('fetchById', () => {
        it('should call the correct query', async () => {
            ...
            expect(graphQLClient.request).toHaveBeenCalledWith(AllProductsDocument, {id})
        })

        it('should return the product matching given id', async () => {
            const graphQLClient = new GraphQLClient('test', 'http://test.com')
            const product = {id: 'testId'}
            graphQLClient.request = jest.fn().mockResolvedValue({ products: { nodes: [product] } })
            const testResource = new ProductsResource(graphQLClient)
            const id = 'testId'
        ...
        it('should return null if id is not matched', async () => {
            ...
            const testResource = new ProductsResource(graphQLClient)
            const id = 'testId'
            const res = await testResource.fetchById(id)
            expect(res).toBeNull()
        })
    })
})